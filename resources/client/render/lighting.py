...
def get_light_level(light_map: dict[int, np.ndarray[Any, np.dtype[np.uint8]]], x: int, y: int) -> float:
    """获取世界坐标 (x, y) 处的归一化光照值。

    参数:
        light_map: 光照图字典 {区块x索引: 2D光照数组}
        x: 世界 X 坐标
        y: 世界 Y 坐标
...
def get_light_levels_at(light_map: dict[int, np.ndarray[Any, np.dtype[np.uint8]]], x: int, y: int) -> tuple[float, float, float, float]:
    """获取方块四个角落的平滑光照值。
...
def color_tint(image: pygame.Surface, new_color: tuple[int, int, int]) -> pygame.Surface:
    ...
    参数:
        image: 原始图像 Surface
        new_color: 目标颜色 (R, G, B)

    返回:
        染色后的新 Surface（原始图像不会被修改）
    """
