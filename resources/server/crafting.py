...
def _matches_ingredient(item_id: str | None, ingredient: dict) -> bool:
    if isinstance(ingredient, list):
        return any(_matches_ingredient(item_id, option) for option in ingredient)
    ...
    if tag == "stone_tool_materials":
        return item_id in {"stone", "cobblestone", "blackstone"}
    if tag == "stone_crafting_materials":
        return item_id in {"stone", "cobblestone", "blackstone"}
    return False


def _trim(grid: list[list[str | None]]) -> list[list[str | None]]:
...
def find_recipe(stacks: list[ItemStack], width: int, height: int) -> tuple[ItemStack, list[int]] | None:
    ...
    grid = [[_item_id(stacks[row * width + col]) for col in range(width)] for row in range(height)]
    compact = _trim(grid)
    if not compact:
        return None
        ...
        if recipe["type"] == "minecraft:crafting_shaped":
            ...
            for row, line in enumerate(pattern):
                for col, char in enumerate(line):
                    item_id = compact[row][col]
                    if char == " ":
                        valid = valid and item_id is None
                    else:
                        valid = valid and _matches_ingredient(item_id, key.get(char, {}))
                    if not valid:
                        break
                if not valid:
                    break
            ...
            present = [item for row in compact for item in row if item is not None]
            ingredients = recipe.get("ingredients", [])
            if len(present) != len(ingredients):
                continue
            remaining = present[:]
            for ingredient in ingredients:
                index = next((i for i, item_id in enumerate(remaining) if _matches_ingredient(item_id, ingredient)), -1)
                if index < 0:
                    break
                remaining.pop(index)
            if remaining:
