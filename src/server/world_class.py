...
from src.server.blocks import AIR, FIRE, SNOW
from src.server.damange_type import EXPLOSION, PLAYER_EXPLOSION
from src.server.entity import Entity
from src.server.generator import Generator
from src.server.location import Location, Vector, decide_x_or_loc
from src.server.particles import (
...
class Chunk:
    ...
    def _recalculate_internal(self):
        ...
        from src.server.light_manager import calculate_light_layers_2d
...
class World:
    ...
    def spawn_experience(self, x: float, y: float, z: int, amount: int):
        from src.server.entities.experience_orb import ExperienceOrb
    ...
    def _restore_entities_for_chunk(self, rx: int) -> None:
        ...
        if not records:
            return
        from src.server.entity_registry import create_entity_from_save
    ...
    def serialize_entities_in_chunks(self, chunk_rxs) -> dict[int, list[dict]]:
        from src.server.entity_registry import is_entity_persistent
